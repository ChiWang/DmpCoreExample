 ...
 *----------------------------------------------------------
 *  Setup install method for controlFunctionOfParentClass
 *   1. if you want to install the whole controlFunctionOfParentClass, execute
 *       scons
...
def useBoostPython(aEnv):
    ...
    aEnv.Append(LIBS=['boost_python-mt'])
...
#--------------------------------------------------------------------
project="controlFunctionOfParentClass"    # or you can set an ABBREVIATION
prefix=project+'Install'
'''
if os.environ.has_key(project+'SYS'):
    prefix=os.environ[project+'SYS']
else:
    UsrPrefix=raw_input('Where to install controlFunctionOfParentClass (press Enter to use default "%s"): ' %prefix)
    if UsrPrefix is not None:
        prefix=UsrPrefix
'''
prefix=os.path.abspath(prefix)
print "install controlFunctionOfParentClass into: ", prefix
